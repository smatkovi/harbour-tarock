...
Item {
    ...
    readonly property int count: steps ? steps.length : 0
    readonly property var step: (running && index >= 0 && index < count) ? steps[index] : null
    readonly property bool lastStep: index >= count - 1
    readonly property real pad: Style.paddingSmall
