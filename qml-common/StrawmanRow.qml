...
Item {
    ...
    readonly property int count: packets ? packets.length : 0
    readonly property real cardHeight: Math.min(maxCardHeight,
                                                (width - 2 * Style.paddingSmall)
                                                / Math.max(1, count) * cardRatio * 0.82)
    readonly property real cardWidth: cardHeight / cardRatio

    readonly property int takenCount: taken ? taken.length : 0
    readonly property real takenHeight: takenCount > 0 ? cardHeight * 0.55 : 0
